...
export default async function PitchDeckPage({ params }: { params: Promise<{ teamId: string }> }) {
  ...
  return (
    <ToolLayout icon={Presentation} title="Pitch Deck Builder" description="A structured outline for your investor pitch deck. This generates the narrative content — you build the slides." teamId={teamId} artifactType="pitch_deck" platformRole={ctx.effectiveRole} currentUserId={ctx.user.id} adminNotes={ctx.adminNotes} mentorNotes={ctx.mentorNotes} artifactId={artifact?.id ?? null}>
