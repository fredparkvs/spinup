...
export default async function UnitEconomicsPage({ params }: { params: Promise<{ teamId: string }> }) {
  ...
  return (
    <ToolLayout icon={Calculator} title="Unit Economics Calculator" description="Calculate CAC, LTV, gross margin, and break-even point. Healthy unit economics must precede scaling." teamId={teamId} artifactType="unit_economics" platformRole={ctx.effectiveRole} currentUserId={ctx.user.id} adminNotes={ctx.adminNotes} mentorNotes={ctx.mentorNotes} artifactId={artifact?.id ?? null}>
